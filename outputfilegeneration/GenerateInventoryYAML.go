...
func GenerateInventoryYAML(
	...
	for _, cluster := range internalData.Clusters {
		...
		for _, hostInfo := range cluster.Masters {
			inventoryAnsibleMasterGroupHosts[hostInfo.ProxmoxVmName+"."+hostInfo.Domain] = ansibleInventoryHostType{
		...
		for _, hostInfo := range cluster.Workers {
			inventoryAnsibleWorkerGroupHosts[hostInfo.ProxmoxVmName+"."+hostInfo.Domain] = ansibleInventoryHostType{
				AnsibleHost: hostInfo.IP,
			}
		}
